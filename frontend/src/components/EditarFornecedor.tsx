...
const EditarFornecedor = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/fornecedores/${id}`)
      .then(res => setFornecedor(res.data))
      .catch(() => setMensagem('❌ Erro ao carregar fornecedor.'))
  }, [id])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setMensagem('')
    try {
      await axios.put(`http://localhost:5000/fornecedores/${id}`, fornecedor)
